//! GPU renderer for the single-input KB4 fisheye mono pipeline.
//!
//! Mirrors [`super::renderer::Renderer`] but for one raw, uncorrected
//! fisheye camera and the KB4 mono shader (`shaders/kb4_mono.wgsl`)
//! instead of the L-shape stitching shader or the (now unused for
//! rendering) cylindrical model - see
//! [`crate::projection::project_world_point`] for the projection math
//! this mirrors in WGSL. Reuses `Renderer`'s plane/texture setup and
...
use crate::projection::MONO_CAMERA_REST_POSITION;

use bytemuck::{Pod, Zeroable};

/// Uniform buffer layout - must match `Kb4MonoUniforms` in
/// `kb4_mono.wgsl` exactly (112 bytes: a `mat4x4<f32>` plus two
/// `vec4<f32>`s plus 4 `f32`s, already 16-byte aligned).
#[repr(C)]
#[derive(Copy, Clone, Debug, Pod, Zeroable)]
struct Kb4MonoUniformsGpu {
    view: [[f32; 4]; 4],
    intrinsics: [f32; 4],
    dist: [f32; 4],
    v_fov: f32,
...
/// The GPU renderer for the single-camera KB4 fisheye projection.
///
...
impl MonoRenderer {
    ...
    pub(crate) fn new(
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("kb4_mono"),
            source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/kb4_mono.wgsl").into()),
        });

        // Same texture bind group layout shape as the stereo `Renderer`
        // (Y/U/V planes + sampler) - `kb4_mono.wgsl`'s `@group(0)`
    ...
    /// Render the KB4-undistorted, pan/tilt/zoomed crop to the internal
    /// target.
    ///
    /// `yaw`/`pitch` position the virtual camera; `rig_tilt`/`rig_roll`
    /// correct for a physically un-level mount (same convention as the
    /// stereo path's `view_matrix`) - typically seeded from the mono
    /// self-calibration's solved rotation so the rest pose looks level
    /// and forward-facing. `camera` is the calibrated KB4 intrinsics;
    /// `max_theta_rad` is the max angle from the optical axis this
    /// calibration is trusted for (see `kb4_mono.wgsl`'s doc comment).
    ...
    pub(crate) fn render_to_target(
        ...
        // to transform world points into camera space. The shader
        // instead rotates a camera-space ray *into* the real camera's
        // optical frame (`ray_w = u.view * ray`), so it needs the
        // inverse - which, for a pure rotation (camera at the origin,
        // no translation), is just the transpose.
        let world_to_view = view_matrix(&MONO_CAMERA_REST_POSITION, yaw, pitch, rig_tilt, rig_roll);
        ...
        let uniforms = Kb4MonoUniformsGpu {
...
mod tests {
    ...
    /// `Kb4MonoUniforms` struct expects (112 bytes, 16-byte aligned) -
    /// a mismatch here would silently corrupt the shader's view of the
    /// buffer instead of failing loudly.
    #[test]
    fn kb4_mono_uniforms_gpu_matches_wgsl_layout() {
        assert_eq!(std::mem::size_of::<Kb4MonoUniformsGpu>(), 112);
        assert_eq!(std::mem::size_of::<Kb4MonoUniformsGpu>() % 16, 0);
    }
}
