...
pub fn run_calibrate_mono(
    ...
    let html_path = match points_arg {
        // Points already supplied (e.g. re-running the solve on a
        // previously-saved points file) - skip the browser round trip.
        Some(_) => None,
        None => Some(write_editor_html(
            &data_uri, width, height, &labels_json,
        )?),
    };

    let raw_points_json = if let Some(path) = points_arg {
        std::fs::read_to_string(path)
            .map_err(|e| anyhow::anyhow!("reading {path}: {e}"))?
    } else {
        let html_path = html_path.expect("checked above");
        open::that(&html_path)
        ...
        read_points_from_stdin_with_retry()?
    };

    let clicked = parse_clicked_points(&raw_points_json)?;
    anyhow::ensure!(
        clicked.len() >= 8,
        "only {} points clicked - need at least 8 for a stable solve (9 unknowns)",
        clicked.len()
    );
